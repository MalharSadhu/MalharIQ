...
\
### 4. (Optional) Run Local LLM for Copilot & AI Synthesis
MalharIQ uses a lightweight local LLM (\llama3.2:1b\) for unstructured document parsing and the advisory copilot.
1. Download and install [Ollama](https://ollama.com/).
2. Pull and start the model in your terminal:
\\ash
ollama run llama3.2:1b
\> **Note:** If Ollama is not installed or running, MalharIQ automatically defaults to robust deterministic rule-based synthesis—the app will **never** crash or freeze.
...
### 2. Configure Analytical Directive
* Review the raw data profile (total records, total dimensions, missing data points).
...
### 3. Run the Autonomous Advisory Pipeline
...
* The status tracker will sequentially display progress through:
  ...
  5. Statistical Quality Gates (KMO, Bartlett's test, Factor loadings).
  6. Multi-architecture algorithm tournaments and validation benchmarking.
  7. Client-ready document synthesis.

### 4. Explore Advisory Workspaces & Export Deliverables
...
* Standardizes structured tabular sheets (\.csv\, \.xlsx\, \.xls\).
* Converts unstructured text, PDFs, logs, and image channel statistics into structured tabular schemas via local LLM parsing (\llama3.2:1b\) with zero cloud data leakage.

### 2. Autonomous Sparse Data Synthesizer
* Micro-datasets (e.g., single text files or small proof-of-concept uploads) crash standard train/validation/test holdout splits.
* MalharIQ detects sparse datasets (< 30 samples) and bootstraps statistically consistent records up to 100 samples, preserving numerical covariance, Gaussian variance, and categorical distribution frequencies.

### 3. Strict 70 / 15 / 15 Partitioning Architecture
* Traditional ML workflows overfit by evaluating solely on training or single test sets.
* MalharIQ strictly isolates:
...
* **Bartlett's Test of Sphericity:** Confirms that correlation matrices are statistically non-orthogonal (p < 0.05).
...
* Evaluates adverse impact across sensitive demographic and cohort proxies using the **EEOC 80% Four-Fifths Rule**.
* Flags selection parity alerts and dynamically computes disparity ratios against the regulatory 0.80 benchmark floor.
...
* Programmatically compiles and embeds empirical diagnostic charts into downloadable executive reports:
  * Formatted multi-section **PDF Briefs** (\pdf2\).
...
1. **Eliminates Algorithmic Silos:** Bridges the gap between raw data and executive decision-making by replacing manual notebook experimentation with an end-to-end automated pipeline[cite: 1].
...
3. **Multi-Modal Flexibility:** Unifies structured CSV/Excel parsing and unstructured text/PDF/image ingestion into a single workflow[cite: 1].
4. **Built-In Regulatory Protection:** Embeds quantitative algorithmic governance (EEOC 80% rule) directly into the model promotion lifecycle[cite: 1].
...
6. **Instant Executive Communication:** Generates board-ready PowerPoint decks and PDF memos with embedded diagnostic plots in seconds, eliminating manual reporting overhead[cite: 1].
